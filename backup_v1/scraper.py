...
DAY_TYPE_RE   = re.compile(r"^(平日|土日祝?|金土(日)?祝(祝前)?|金土祝前|祝前)$")
PREF_RE       = re.compile(
...
async def collect_all_shop_urls(page: Page) -> list[dict]:
    ...
    待機戦略:
      1. networkidle で初期JS完了を待つ
      2. /shop/XXX/ 形式のリンクが1件以上現れるまで最大 LIST_TIMEOUT ms 待機
...
def extract_price_table(soup: BeautifulSoup) -> list[dict]:
    ...
    if price_section:
        ...
        # 念のため、imgのalt属性に「料金表」が含まれる見出しも探す
        for tag in soup.find_all(["h2", "h3", "h4", "h5"]):
            img = tag.find("img")
            if img and "料金表" in img.get("alt", ""):
                price_heading = tag
                break
...
def main():
    ...
    if args.dry_run:
        ...
        # この場合のみ Playwright での収集結果をそのまま stdout に出す
        async def _dry():
            async with async_playwright() as pw:
